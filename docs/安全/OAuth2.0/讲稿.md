...
简单明了：用oauth，比较隐晦的前提是你有被很多别人利用的价值。
...
oauth，现在大家看到的都是oauth2 ，这里主要讲oauth2。

oauth1和oauth2的最大区别，就是流程的改进，变得更全面，更安全。这个我们后面讲oauth2的授权模式的就能体会了。

至于spring security是什么？

​	1、来自大佬们的智慧结晶，功能强大，spring 默认组件
...
所以，oauth2.0的定位是有一种授权协议，保证第三方只有获得授权后，才可以进一步操作授权者的数据。换一种说法，也可以oauth2叫做安全协议。
